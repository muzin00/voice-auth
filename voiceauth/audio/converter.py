...
from voiceauth.engine.exceptions import AudioConversionError
from voiceauth.engine.settings import settings


class AudioConverter:
    """Audio converter using PyAV.

    Implements AudioConverterProtocol from voiceauth.domain.protocols.audio.
    """
    ...
    def ensure_mono(self, audio: np.ndarray) -> np.ndarray:
        ...
        if audio.ndim == 2:
            return np.mean(audio, axis=0 if audio.shape[0] <= 2 else 1).astype(
                np.float32
            )
