...
| ID     | 機能                 | 説明                                      |
| ------ | -------------------- | ----------------------------------------- |
| FR-8-1 | 音声入力（マイク）   | ブラウザのMediaRecorder APIで録音         |
| FR-8-2 | 音声入力（ファイル） | wav/mp3/m4a等の音声ファイルをアップロード |
| FR-8-3 | 話者登録デモ         | speaker_id入力 + 音声で登録APIを呼び出し  |
| FR-8-4 | 認証デモ             | speaker_id入力 + 音声で認証APIを呼び出し  |
| FR-8-5 | 結果表示             | 登録/認証結果をリアルタイムで画面に表示   |
