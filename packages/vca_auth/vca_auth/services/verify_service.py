...
class VerifyService:
    ...
    def start_verification(
        ...
        if not self.speaker_repository.speaker_exists(speaker_id):
            raise SpeakerNotFoundError(f"Speaker '{speaker_id}' not found")

        # Check if speaker has PIN for fallback
        speaker = self.speaker_repository.get_speaker_by_id(speaker_id)
    ...
    def verify_voice(
        ...
        try:
            ...
            registered_embeddings = self.speaker_repository.get_voiceprints(
                session.speaker_id
            )
    ...
    def verify_pin(
        ...
        speaker = self.speaker_repository.get_speaker_by_id(session.speaker_id)
